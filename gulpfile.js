...
function notify(options) {
    var notifyer = require('node-notifier');
    var notifyOptions = {
        sound: 'Bottle',
...
function startTests(singleRun, done) {
    ...
    if (args.startServers) { // gulp test --startServers
        log('Starting server');
        var saveEnv = process.env;
        saveEnv.NODE_ENV = 'dev';
        saveEnv.PORT = 8888;
    ...
    function karmaCompleted(karmaResult) {
        ...
        if (karmaResult === 1) {
            done('karma: tests failes with code ' + karmaResult);
        } else {
            done();
        }
    }
